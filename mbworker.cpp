...
void MBWorker::init()
{
    ...
    for(i=1;i<pass_count;i++){
        ...
        prevp=dynamic_cast<Passenger*>(prevp->next);
        *rww>>*prevp;
    }
    last_p=prevp;
    ...
    if(train_count>0){
        ...
        *rww>>*first_t;
        ...
        for(int j=1;j<first_t->car_count;j++){
            ...
            prevc=dynamic_cast<Car*>(prevc->next);
            *rww>>*prevc;
        }
        first_t->last=prevc;
    }
    ...
    for(i=1;i<train_count;i++){
        ...
        prevt=dynamic_cast<Train*>(prevp->next);
        prevt->routs=(dynamic_cast<Train*>(prevt->prev))->routs;
        *rww>>*prevt;
...
void MBWorker::save()
{
    ...
    while (curp!=NULL) {
        *rww<<*curp;
        if(curp->next!=NULL){
        curp=dynamic_cast<Passenger*>(curp->next);
        delete curp->prev;
    ...
    while(curt!=NULL){
        *rww<<*curt;
        ...
        while(curc!=NULL){
            *rww<<*curc;
            if(curc->next!=NULL){
            curc=dynamic_cast<Car*>(curc->next);
            delete curc->prev;
        ...
        curt=dynamic_cast<Train*>(curt->next);
        delete curt->prev;
        }else{
            delete curt; break;}
    }
...
void MBWorker::sell(){
    ...
    while(cur!=NULL){
        cout<<*cur;
        cur=dynamic_cast<Train*>(cur->next);
    }
...
void MBWorker::del_train()
{
    ...
    while (pas!=NULL) {
        if(pas->train_number==tr_n){
            if(pas->next!=NULL){
            pas=dynamic_cast<Passenger*>(pas->next);
            if(pas==last_p)
                last_p=dynamic_cast<Passenger*>(pas->prev);
            if(pas==first_p)
                first_p=dynamic_cast<Passenger*>(pas->next);
            delete pas->prev;
            }else{
                if(pas==last_p)
                        last_p=dynamic_cast<Passenger*>(pas->prev);
    ...
    if(cur==last_t)
        last_t=dynamic_cast<Train*>(cur->prev);
    if(cur==first_t)
        first_t=dynamic_cast<Train*>(cur->next);
    delete cur;
    train_count--;
    updateTrainCarFirstP();
}
...
void MBWorker::del_pas()
{
    ...
    if(cur==last_p)
        last_p=dynamic_cast<Passenger*>(cur->prev);
    if(cur==first_p)
        first_p=dynamic_cast<Passenger*>(cur->next);
    delete cur;
    pass_count--;
    updateTrainCarFirstP();
}
...
void MBWorker::cor_train_num(Train *tr)
{
    ...
    while(ccar!=NULL){
        ...
        ccar=dynamic_cast<Car*>(ccar->next);
    }
